...
def test_bad_boids_regression():
    ...
    for index, boid in enumerate(boids.boids):
        assert_almost_equal(boid.position[0], regression_data["after"][index].position[0], delta=0.01)
        assert_almost_equal(boid.position[1], regression_data["after"][index].position[1], delta=0.01)
        assert_almost_equal(boid.velocity[0], regression_data["after"][index].velocity[0], delta=0.01)
        assert_almost_equal(boid.velocity[1], regression_data["after"][index].velocity[1], delta=0.01)
